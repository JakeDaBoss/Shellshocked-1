...
local imageActive   = "cwmetrorp/station_menu_2.png"
local imageDisabled = "cwmetrorp/station_menu_0.png"
	...
	if (!Clockwork.theme:Call("PreMainMenuRebuild", self)) then
		...
		for k, v in pairs(Clockwork.menuitems.stored) do
			...
			if (!panel.IsButtonVisible or panel:IsButtonVisible()) then
				button = vgui.Create("cwMetroButton", self);
				image  = vgui.Create("DImage", self )
				sound = true 
...
function PANEL:Paint(w, h)
	...
	Clockwork.kernel:DrawGradient(
		GRADIENT_RIGHT, 0, 0, scrW * 0.3, scrH, Color(0, 0, 0, 250)
	);
	...
	if (!Clockwork.theme:Call("PreMainMenuThink", self)) then
		...
		for k, v in pairs(Clockwork.menu:GetItems()) do
			...
			if (IsValid(v.button)) then
				...
				if (IsValid(v.image)) then
					if (v.panel == activePanel) then
						v.image:SetImage( imageActive );
					else
						v.image:SetImage( imageDisabled );
					end;
					
					if ( v.button:GetHovered() ) then						
						v.image:SetImage( imageActive );
					end;
				end;
