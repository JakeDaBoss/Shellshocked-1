	...
	for k, itemTable in pairs(Clockwork.item:GetAll()) do
		...
		if (spawnValue and spawnValue > 0) then
			...
			if (rareItem) then
				...
				key = key + 1;
				-- Add in item
				rareItems[key] = itemTable("uniqueID");
			...
			elseif (spawnType and self.typeValues[spawnType]) then
				...
				-- Add in current item
				itemsLists[spawnType][nxt] = itemTable("uniqueID");
				-- Set latest key
...
function PLUGIN:SpawnItems(amount)
	...
	for i = 1, amount do
		...
		if (math.random() > rareSpawnChance) then
			...
			if (randomItem) then
				local itemTable = Clockwork.item:FindByID(randomItem);
				if (itemTable) then
		...
		if (randomItem) then
			randomItem = Clockwork.item:CreateInstance(randomItem);
		...
		if (randomItem.OnItemSpawnerSpawn) then
			randomItem:OnItemSpawnerSpawn();
		end;
